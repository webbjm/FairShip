...
class SNDVertex:
 ...
 def __init__(self,fout):
  ...
  self.fitTrackLoc = "genfit_tracks"#"FitTracks"
 ...
 def TwoTrackVertex(self):
  ...
  if fittedTracks.size() < 2 or fittedTracks[0].getFitStatus().getChi2()<0 or fittedTracks[1].getFitStatus().getChi2()<0: 
      print("Track list too small, or chi2 too low")
      return
  particles    = self.fPartArray
  PosDirCharge,CovMat,scalFac = {},{},{}
  for tr, track in enumerate(fittedTracks):
...
#   HNL true
     mctrack =0
     for true_track in self.sTree.MCTrack:
         if true_track.GetMotherId() in [-1]:
             mctrack = true_track
...
#     plane = ROOT.genfit.RKTrackRep().makePlane(ROOT.TVector3(0,0,HNLPos[2]),ROOT.TVector3(1,0,0),ROOT.TVector3(0,1,0))
     top = ROOT.gGeoManager.GetTopVolume()
      ...
      #  e_ecal = ecal.GetMatrix().GetTranslation()[2]
...
#     ROOT.genfit.tools.invertMatrix(cov,covInv)
     ROOT.gInterpreter.Declare('#include "Tools.h"')
     ROOT.genfit.tools.invertMatrix(cov,covInv)
     ...
     yFit = values[1]
     zFit = values[2]
     print(f"vertex_z: true, fit, diff: {mctrack.GetStartZ()}, {zFit}, {mctrack.GetStartZ()-zFit}")
...
#     particle = ROOT.ShipParticle(13,0,-1,-1,t1,t2,P,vx)
...
# newPos,doca = myVertexError(t1,t2,PosDirCharge)
 def finish(self):
  ...
  self.sTree.Write()
  ut.writeHists(self.h,"recohists.root")
#  ut.errorSummary()
