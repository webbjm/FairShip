...
//    TGeoVolume *detector = gGeoManager->FindVolumeFast("Detector");
    TGeoVolumeAssembly *detector = new TGeoVolumeAssembly("detector");
    top->AddNode(detector, 1, new TGeoTranslation(0,0,-31 * m)); //Set position 3m down stream
...
//    AddSensitiveVolume(SensorVolume);
    ...
    double offset_x = 0 * cm; // +35 * cm;
    double offset_y = 0 * cm; //-15 * cm;
...
//        std::cout<<"Entering vol: "<< pdgCode <<std::endl;
    	fELoss = 0.;
...
//if(fELoss != 0.){    std::cout<<"En Loss: "<<fELoss<<std::endl;}
//    printf("%.17g\n",fELoss);
...
//	std::cout<<strip_id<<" "<< sensor_id<<" "<<fVolumeID<<std::endl;
        ...
        AddHit(fTrackID,
               ...
               fELoss,
               pdgCode);
//        std::cout<<"Point Added: "<< fTrackID <<std::endl;
